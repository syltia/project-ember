...
apt install git curl unzip p7zip-full sudo tmux net-tools ufw php php-mysqli -y

echo "--- 1b. Configuration des règles du pare-feu (UFW) ---"
echo "Ajout des règles pour SSH et les ports du serveur WoW (le pare-feu n'est pas encore activé)."
...
echo "--- 3. Activation du pare-feu (UFW) ---"
echo "Le pare-feu va être activé avec les règles SSH + AzerothCore définies à l'étape 1."
echo "Activation dans 5 secondes..."
sleep 5
ufw --force enable
...
echo "--- 8. Téléchargement du script finalize Ember ---"
